...
def follow_likers(user: User):
  ...
  def getMedia() -> instagrapi.types.Media:
    potential_media: list[instagrapi.types.Media] = [
        session.user_medias(session.user_id_from_username(name),
                            1)[0] for name in account_usernames
    ]
  ...
  while len(people_followed) < amount:
    ...
    for person in people:
      ...
      to_follow = all(
          [
              int(person.pk) not in l for l in
              ( followers,
                followed,
                people_followed,
                people_ignored )
      ...
      if quality and to_follow:
        ...
        if person.following_count >= 350:
          to_follow = False
                                                                             # follow
      if to_follow:
        people_followed.append(int(person.pk)
                               ) if session.user_follow(person.pk) else None
      else:
...
def unfollow_users(user: User):
  session = user.login()
  peopleid: list[int] = user.userdata[
      "following"][:user.usercnfg["interation_session_limit"]:]
  people_unfollowed = [
      personid for personid in peopleid
      if session.user_unfollow(str(personid))
  ]
