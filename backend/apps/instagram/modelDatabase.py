...
from ...utils.cleanup import TrashManager
from .modelSchemas import PostData, PostMedia, UserConfig, UserMetadataExtra
from .utilities import generate_new_device
...
class User(BaseUser):
    device: dict[str, typing.Any] = fields.JSONField(
        null=True, default=generate_new_device
    )
...
class Post(BaseDB):
    ...
    async def build(self) -> None:
        ...
        self.caption = "\n".join(
            [
                each_line.replace("${text}", self.kwargs.caption_text)
                .replace(
...
POST_PYDANTIC = pydantic_model_creator(Post, name="Post", exclude_readonly=True)
