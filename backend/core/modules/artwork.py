...
class image(pydantic.BaseModel):
  EXTENSION: typing.ClassVar[str] = "jpg"
  SIZE: typing.ClassVar[dict[str,
                             tuple[int,
                                   int]]] = {
                                       "1:1": ( 1920,
                                                1920 ),
  ...
  RENDER_SETTINGS: typing.ClassVar[dict[str,
                                        typing.Any]] = {
                                            "fp": None,
                                            "format": "JPEG",
                                            "resample": 1,
  ...
  def crop(self, size: tuple[int, int], force_method: str = None):
  ...
  def resize(
      self,
      parameter: typing.Literal["height",
                                "width"],
      parameter_value: int
  ):
  ...
  def add_text(
    ...
    mask = mask.rotate(
        angle,
        resample=self.Ex.Image.ANTIALIAS,
        expand=True,
        fillcolor=( 0,
                    0,
                    0,
  ...
  def set_ratio(
      self,
      ratio: str,
      background: typing.Literal["solid",
                                 "blur"]
  ) -> image:
    ...
    larger_proportion: typing.Literal[
        "height",
        "width"] = ("height" if self.orientation.isPotrait else "width")
    ...
    if background == "solid":
      ...
      for index in range(10):
        ...
        avg_list.append(
            list(np.average(img_matrix[rows - index - 1],
                            axis=0))
        )                                                                  # bottom
        ...
        try:
          avg_list.append(
              list(np.average(img_matrix[...][cols - index - 1],
                              axis=0))
          )
...
class video(pydantic.BaseModel):
  ...
  def add_rounded_corners(self, edge_radius: int):
    ...
    mask = image(img=image.Ex.Image.new("RGB",
                                        self.vid.size)
                 ).add_rounded_corners(edge_radius,
                                       True)
    self.vid = self.vid.set_mask(
        self.Ex.ImageClip(np.array(mask.img.convert("RGB")),
                          ismask=True)
    )
    return self
  ...
  def add_text(
      ...
      xy: tuple[float,
                float],
      text: str,
      font: image.Ex.ImageFont.ImageFont
  ):
    """Applies text to self object, the text can be customised.
        The angle of rotation is in anti - clockwise direction."""
    im = (
        image(img=image.Ex.Image.new("RGBA",
                                     self.vid.size,
                                     ( 0,
                                       0,
                                       0,
    ...
    clip = video.Ex.ImageClip(np.array(im),
                              duration=self.vid.duration).set_fps(
                                  self.vid.fps
                              )
...
def typecheck(data: str | bytes) -> typing.Type[video | image]:
  ...
  try:
    image.Ex.Image.open(data) if isinstance(data,
                                            str) else image.Ex.Image.open(
                                                io.BytesIO(data)
                                            )
...
def font_loader(filename: str, size: int):
  ...
  url = FONT_LINK.format(
      "-".join(re.findall(r"[A-Z](?:[a-z]+|[A-Z]*(?=[A-Z]|$))",
                          fontName))
  ).lower()
