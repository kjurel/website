...
class Builder(pydantic.BaseModel):
  ...
  def content(self) -> tuple[list[bytes], list[str]]:
    ...
    if (
        ...
        getattr(self.__class__,
                f"{attrname}_content_type_{attrindex}")
    ) is None:
      raise ValueError
  ...
  def usertags(self):
    ...
    return [
        x for i,
        x in enumerate(all_usertags) if i < self.data.usertags_amount
    ]
    ...
    for im, offset in zip(imgs, ( 0, 960 )):
      bg = (
          image(img=im.copy()
                        ...
                        960 - BD * 2),
                       "retain height").add_rounded_corners(25).get_copy
      )
      fg = (
          image(img=im.copy()).resize("height",
                                      450).add_rounded_corners(25).get_copy
      )
      COMP.paste(
  ...
  def Fanarts_content_type_0(self) -> list[image]:
    return [
        obj.set_ratio("1:1",
                      "solid") for obj in self.media if isinstance(obj,
                                                                   image)
    ]
  ...
  def Fanarts_content_type_1(self) -> list[image]:                            # bottom_bar
    ...
    def draw_box(draw: image.Ex.ImageDraw.ImageDraw):
      draw.rounded_rectangle(
          xy=(
              (0 + BOX_OUTLINE_RADIUS,
               1920 - BOX_HEIGHT),
              (1920 - BOX_OUTLINE_RADIUS,
               1920 + BOX_HEIGHT),
          ),
          radius=BOX_RADIUS,
          fill=( 0,
    ...
    def draw_text(draw: image.Ex.ImageDraw.ImageDraw):
      ...
      draw.text(
          (TEXT_RULER_X + light_font.getsize("Created by ")[0],
           TEXT_RULER_Y),
          aut,
          "white",
          dark_font,
      ...
      draw.text(
          (1920 - TEXT_RULER_X - wooble_font.getsize(src)[0],
           TEXT_RULER_Y),
          src,
          "white",
          wooble_font,
    ...
    def draw_icon(img: image.Ex.Image.Image):
      ...
      icon = (
          image(img=icon.convert("RGBA")
                         ...
                         ICON_SIZE).add_rounded_corners(ICON_CORNER).get_copy
      )
      
      img.paste(
          icon,
          (
              1920 - TEXT_RULER_X - wooble_font.getsize(src)[0] - ICON_SIZE -
              ICON_GAP,
