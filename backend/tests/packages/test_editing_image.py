...
def test_image_validation():
  ...
  assert image(img=image.Ex.Image.open(io.BytesIO(T1.get_bytes))
               ) == image(img=img)
...
def test_image_orientation():
  ...
  assert not po.orientation.isLandscape and po.orientation.isPotrait and not po.orientation.isSquare
  assert la.orientation.isLandscape and not la.orientation.isPotrait and not la.orientation.isSquare
  assert not sq.orientation.isLandscape and not sq.orientation.isPotrait and sq.orientation.isSquare
