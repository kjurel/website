...
async def test_user(client: FastAPI, tortoise):
    ...
    async with AsyncClient(app=client, base_url="http://localhost") as ac:
        ...
        r = await ac.post(
            ...
            auth=(environ["MASTER_USER"], environ["MASTER_PASS"]),
        )
...
def delete_user():
    headers = {
        "accept": "application/json",
        "Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJyZXlvbWVuc3VrdW5hIiwiZXhwIjoxNjUzMzIzMjY0fQ.u5QlWxMR1qIcshIxU8yy5orYSSKMb3nP4Xo7a0TU8NI",
    }

    response = requests.delete(
        "http://localhost:8000/api/instagram/u/", headers=headers
    )
...
def get_user():
    headers = {
        "accept": "application/json",
        "Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJyZXlvbWVuc3VrdW5hIiwiZXhwIjoxNjUzMzI0OTI1fQ.7keVlAZC4tyl3X3onHmof-0KlxVAC_qVYxVc7PGTt3k",
    }

    response = requests.get("http://localhost:8000/api/instagram/u/", headers=headers)
