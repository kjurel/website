...
class image(pydantic.BaseModel):
    ...
    def crop(self, size: tuple[int, int], force_method: t.Optional[str] = None):
        ...
        elif (force_method == "protect height") or (
            force_method is None and self.orientation.isLandscape
        ):
    ...
    def add_rounded_corners(
        ...
        self.Ex.ImageDraw.Draw(crvmask).rounded_rectangle(
            ((0, 0), imgmask.size), edge_radius, "white"
        )
        ...
        if ol_kwargs is not None:  # {"outline":None, "width":None}
            ...
            self.Ex.ImageDraw.Draw(outline).rounded_rectangle(
                ((0, 0), mask.size), edge_radius, (0, 0, 0, 0), **ol_kwargs
            )
    ...
    def add_text(
        ...
        self.Ex.ImageDraw.Draw(mask).text(
            (0, 0), text, "#FFFFFF", font, stroke_width=3, stroke_fill="#000000"
        )
        mask = mask.rotate(
    ...
    def set_ratio(
        self, ratio: str, background: typing.Literal["solid", "blur"]
    ) -> image:
        ...
        larger_proportion: typing.Literal["height", "width"] = (
            "height" if self.orientation.isPotrait else "width"
        )
        ...
        if background == "solid":
            ...
            for index in range(10):
                ...
                avg_list.append(
                    list(np.average(img_matrix[rows - index - 1], axis=0))
                )  # bottom
                avg_list.append(list(np.average(img_matrix[...][0], axis=0)))  # left
                # rigth
                try:
                    avg_list.append(
                        list(np.average(img_matrix[...][cols - index - 1], axis=0))
                    )
...
class video(pydantic.BaseModel):
    ...
    def add_rounded_corners(self, edge_radius: int):
        ...
        mask = image(img=image.Ex.Image.new("RGB", self.vid.size)).add_rounded_corners(
            edge_radius, True
        )
    ...
    def add_text(
        ...
        im = (
            ...
            .add_text(xy, text, font)
            .get_copy
        )
        clip = video.Ex.ImageClip(np.array(im), duration=self.vid.duration).set_fps(
            self.vid.fps
        )
        self.vid = video.Ex.CompositeVideoClip(
            [self.vid, clip], use_bgclip=True, size=(1920, 1920)
        )
        return self
...
ICON_MAPPING = {
    "devianart": {"url": "devianart.png", "color": "#64DD17"},
    "instagram": {"url": "instagram-new--v1.png", "color": "#D71C61"},
    "reddit": {"url": "reddit.png", "color": "#D84315"},
    "twitter": {"url": "twitter--v1.png", "color": "#4CBAFF"},
}
